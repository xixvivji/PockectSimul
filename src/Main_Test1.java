...
public class Main_Test1 {
    ...
    static final double BALL_ = 5.73;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (socket != null) {
                ...
                // 목표 공 업데이트 - 현재 공이 포켓에 들어갔으면 다음 공으로
                if (!isValidBall(balls[nowTargetBallNumber][0], balls[nowTargetBallNumber][1])) {
                    // 다음 유효한 공 찾기
                    nowTargetBallNumber = -1;
                    for (int i = 1; i < NUMBER_OF_BALLS; i++) {
                        if (isValidBall(balls[i][0], balls[i][1])) {
                            nowTargetBallNumber = i;
                            break;
                        }
                    }
                }
                ...
                if (nowTargetBallNumber > 0 &&
                    ...
                    float hitPoint_x = targetBall_x -
                            (dx_target_hole / dist_target_hole) * (float) (BALL_);
                    float hitPoint_y = targetBall_y -
                            (dy_target_hole / dist_target_hole) * (float) (BALL_);
