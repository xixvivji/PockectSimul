...
public class Main_Test_Complete_Version2 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (socket != null) {
                ...
                // 목적구의 좌표
                if (nowTargetBallNumber < NUMBER_OF_BALLS && isValidBall(balls[nowTargetBallNumber][0], balls[nowTargetBallNumber][1])) {
                    float targetBall_x = balls[nowTargetBallNumber][0];
                    float targetBall_y = balls[nowTargetBallNumber][1];

                    System.out.println("Target Ball: " + nowTargetBallNumber + " at (" + targetBall_x + ", " + targetBall_y + ")");
                    ...
                    System.out.println("Target Hole: " + targetHoleIdx + " at (" + hole_x + ", " + hole_y + ")");

                    // 목표 공을 포켓에 넣기 위한 각도 계산
                    float dx_target_hole = hole_x - targetBall_x;
                    float dy_target_hole = hole_y - targetBall_y;
                    float dist_target_hole = (float)Math.sqrt(dx_target_hole * dx_target_hole + dy_target_hole * dy_target_hole);

                    // 목적구가 포켓에 너무 가까우면 hitPoint를 목적구 위치로 설정
                    float hitPoint_x, hitPoint_y;
                    if (dist_target_hole < BALL_RADIUS * 2) {
                        hitPoint_x = targetBall_x;
                        hitPoint_y = targetBall_y;
                    } else {
                        hitPoint_x = targetBall_x - (dx_target_hole / dist_target_hole) * (float)(BALL_RADIUS * 2);
                        hitPoint_y = targetBall_y - (dy_target_hole / dist_target_hole) * (float)(BALL_RADIUS * 2);
                    }
                    ...
                    // 유효한 목표 공이 없는 경우
                    System.out.println("No valid target ball found.");
                    angle = 0;
                    power = 0;
                }
    ...
    private static int updateTargetBall(float[][] balls, int currentTarget, int order) {
        ...
        if (currentTarget < NUMBER_OF_BALLS && !isValidBall(balls[currentTarget][0], balls[currentTarget][1])) {
            if (order == 1) { // 선공: 홀수 공(1,3,5)
                for (int i = 1; i < NUMBER_OF_BALLS; i += 2) {
                    if (isValidBall(balls[i][0], balls[i][1])) return i;
                }
            } else { // 후공: 짝수 공(2,4) 및 8번(5) 공
                for (int i = 2; i < NUMBER_OF_BALLS; i += 2) {
                    if (isValidBall(balls[i][0], balls[i][1])) return i;
                }
                if (isValidBall(balls[5][0], balls[5][1])) return 5;
            }
